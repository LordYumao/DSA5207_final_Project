...
from work.HypEmo.hypbert_parameter import HypBert
...
# wxq modified: make alpha, gamma trainable
class HypEmo():
    def __init__(self, dataset, n_classes, class_names, idx2vec, batch_size=16):
        ...
        self.model = HypBert(num_labels=args.n_classes)
    ...
    def train_step(self, ith_epoch):
        ...
        # logging.info(f'gamma:{gamma}, alpha:{alpha}')
        logging.info(f'''train | loss: {total_loss/step:.04f} acc: {train_acc:.04f}, f1: {train_weighted_f1:.04f}''')
        
        return {'loss': total_loss/step, 'train_acc': train_acc, 'train_weighted_f1': train_weighted_f1, 'gamma' : gamma, 'alpha' : alpha}
    ...
    def test_step(self, ith_epoch):
        ...
        return {'test_pred': test_pred, 'test_acc': test_acc, 'test_weighted_f1': test_weighted_f1, 'gamma' : self.model.gamma.item(), 'alpha' : self.model.alpha.item()}
