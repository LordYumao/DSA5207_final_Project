...
from work.HypEmo.hypemo_parameter import HypEmo
...
gm = HypEmo(args.dataset, num_classes, class_names, idx2vec, batch_size=args.batch_size)
...
for i in range(args.epochs):
    ...
    train_gamma.extend(train_log['gamma'])
    train_alpha.extend(train_log['alpha'])
    ...
    if test_log['test_weighted_f1'] > best_test_weighted_f1:
        ...
        best_test_alpha = test_log['alpha']
        best_test_gamma = test_log['gamma']
    ...
    logging.info(f"[best] valid | acc: {best_valid_acc:.04f}, f1: {best_valid_weighted_f1:.04f}\n test | acc: {best_test_acc:.04f}, f1: {best_test_weighted_f1:.04f}, alpha: {best_test_alpha}, gamma: {best_test_gamma}")
