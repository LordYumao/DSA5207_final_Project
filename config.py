...
all_dataset_list = ['go_emotion', 'ED', 'ED_easy_4', 'ED_hard_a', 'ED_hard_b', 'ED_hard_c', 'ED_hard_d','se2018_single']
...
config_args = {
    'training_config': {
        ...
        'seed': (4270, 'seed for training'),
        'alpha': (0.9, 'weight for BCE*poincare_dist loss'),
        'gamma': (0.1, 'weight for poincare loss'),
        'k_folds': (5, 'number of folds for cross-validation (default: 5, use 1 for standard train/val split)'),
    },
...
def get_dicts(dataset, return_emb_dicts=False):
    ...
    elif dataset == 'se2018_single':
          # Label-to-index mapping for SemEval-2018 E-c single-label dataset
          label2idx = {
              'anger': 0,
              'anticipation': 1,
              'disgust': 2,
...
# === make label dicts globally available ===
LABEL2ID, ID2LABEL = get_dicts(config_args['data_config']['dataset'][0])
