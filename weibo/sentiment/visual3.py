...
if 'label' in df.columns and 'pred_label' in df.columns:
    ...
    sns.heatmap(cm_df, annot=True, fmt='d', cmap='Blues', xticklabels=all_labels, yticklabels=all_labels)
    ...
    metrics_df = report_df[['precision', 'recall', 'f1-score']].drop(['accuracy'], axis=1)
    ...
    if not errors.empty:
        print("\nwith errors：")
        print(errors[['text', 'label', 'pred_label']].head(10))
